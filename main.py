...
def train(args):
    ...
    Ks = eval(args.Ks) # metric@K [20, 40, 60, 80, 100]
    k_min = min(Ks)
    k_max = max(Ks)
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        time1 = time()
        kg_total_loss = 0
        n_kg_batch = data.n_kg_train // data.kg_batch_size + 1  # 6420520 // 2048 + 1 = 3136
        ...
        # Update adj matrix
        time6 = time()
        model(data.adjacency_matrix, mode='update_adj')
        logging.info('Update Adj: Epoch {:04d} | Total Time {:.1f}s'.format(epoch, time() - time6))        


        # Train CF
        time3 = time()
        cf_total_loss = 0
        n_cf_batch = data.n_cf_train // data.cf_batch_size + 1  # 652514 // 1024 + 1 = 637
        ...
        for iter in range(1, 10):    
            ...
            if (iter % args.cf_print_every) == 0:
                logging.info('CF Training: Epoch {:04d} Iter {:04d} / {:04d} | Time {:.1f}s | Iter Loss {:.4f} | Iter Mean Loss {:.4f}'.format(epoch, iter, n_cf_batch, time() - time4, cf_batch_loss.item(), cf_total_loss / iter))
        logging.info('CF Training: Epoch {:04d} Total Iter {:04d} | Total Time {:.1f}s | Iter Mean Loss {:.4f}'.format(epoch, n_cf_batch, time() - time3, cf_total_loss / n_cf_batch))
